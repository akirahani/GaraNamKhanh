...
<div class="col-lg-12">
    <div class="card">
        <div class="card-body">
            <hr>
            <form action="{{ url('/admin/file/out/insert') }}" method="POST">
                @csrf
                <h3>Tạo phiếu xuất</h3>
                <br>
                <div class="customer-content row" style="margin-left:5%">
                    <div class="customer-info col-4">
                        <label for="input-7">Khách hàng</label>
                        <select class="form-select"   aria-label="Default select example" name="id_customer"  >
                                @foreach($customers as $customer)
                                    <option style="background-color:white" >{{$customer->id}}- {{$customer->name}}- {{$customer->phone}}  </option>
                                @endforeach
                        </select>
                    </div>
                </div>
